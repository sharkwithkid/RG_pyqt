...
def validate_input_sheet_structure(
    ...
    for r in range(data_start_row, last_data_row + 1):
        ...
        if allow_blank_class_for_kindergarten:
            ...
            is_kindergarten = re.sub(
                r"\s+", "", grade_s
            ) in {"유치원", "5세", "6세", "7세", "5세반", "6세반", "7세반"}

            for key, v in row_values.items():
                if key == "class" and is_kindergarten:
                    continue
...
def analyze_roster_once(roster_ws, input_year: int) -> Dict:
    ...
    [roster_time 추정]
    1학년 아이디 prefix 최빈값과 input_year를 비교해 명부 학년도를 추정한다:
      prefix == input_year     → "this_year" (올해 명부, 개학 후)
      prefix == input_year - 1 → "last_year" (작년 명부, 개학 전)
      그 외                    → "unknown"

    단, 최종 roster_time/ref_grade_shift는 scan_pipeline에서
    명부 기준일과 개학일을 비교하여 덮어쓴다. (ID 기반 추정은 참고용)
    """
    ...
    # resolve_transfer_name_conflicts가 이 키/구조를 기대함
    # name_counter_by_grade는 Counter라 .keys()로 이름 목록 추출
...
def scan_pipeline(
    ...
    try:
        ...
        if roster_xlsx:
            if not _xlsx_school_exists(Path(roster_xlsx), school_name, col_map):
                raise ValueError(
                    f"[ERROR] 명단 파일에서 '{school_name}' 학교를 찾을 수 없습니다. "
                    f"(파일: {Path(roster_xlsx).name})"
                )
            sr.roster_xlsx_path = Path(roster_xlsx)
            log(f"[INFO] 명단 파일 검증 통과: {Path(roster_xlsx).name}")
        else:
        ...
        if freshmen_file:
            try:
                ...
                import traceback
                log(f"[ERROR] 신입생 파일 학년 분석 중 오류가 발생했습니다: {e}")
                log(f"[DEBUG] {traceback.format_exc()}")
                raise
        ...
        if need_roster_by_freshmen:
            log("[INFO] 신입생 파일에 1학년 외 학년이 포함되어 있어 학생명부가 필요합니다.")
            ...
            try:
                ...
                if roster_ws is not None and roster_path is not None:
                    ...
                    try:
                        ...
                        id_roster_time = roster_info.roster_time

                        log(f"[INFO] 학생명부 분석 완료 (학년도 추정: {roster_info.roster_time})")
                    except Exception as e:
                        import traceback
                    ...
                    try:
                        ...
                        id_roster_time = roster_info.roster_time if roster_info else "unknown"

                        if id_roster_time == "this_year":
                            log(f"[INFO] 학생명부 학년도: {expected_year}학년도로 추정됩니다.")
                        elif id_roster_time == "last_year":
                            log(f"[INFO] 학생명부 학년도: {expected_year-1}학년도로 추정됩니다.")
                        else:
                            log("[INFO] 학생명부 학년도 추정이 불확실합니다.")
                        ...
                        if id_roster_time in ("this_year", "last_year") and id_roster_time != roster_time:
                            log(
                                "[WARN] 학생명부 학년도 추정 결과가 기준일 기반 예상과 다릅니다. 명부가 최신 파일인지 확인해 주세요."
                            )
                    except Exception as e:
                        import traceback
